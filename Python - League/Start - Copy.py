...
# selectChampionImageDict = {}
# selectableChampionDict = {}

# def displaySelectedChampion(event): #When a champion has been selected, run this event handler
#     global currentPage, someImage, selectedChampion, selectChampionClickedID
#     #get the pos of cursor to determine which champion was clicked
#     selectChampionPositionX = int((selectChampionFrame.winfo_pointerx() - selectChampionFrame.winfo_rootx() - 255) / 100)
#     selectChampionPositionY = int((selectChampionFrame.winfo_pointery() - selectChampionFrame.winfo_rooty() - 146) / 80)
#     selectChampionClickedID = (selectChampionPositionX + selectChampionPositionY * 8) + (currentPage) * 56
...
# def changePage(event): #event handler to show the next page of champions
#     global currentPage
#     if event.delta < 0 and currentPage < pageCount() - 1: #scroll down
#         selectChampionPageDict[currentPage].place_forget()
#         currentPage += 1
#         selectChampionPageDict[currentPage].place(relx = 210/1280, rely = 0.175)
#     elif event.delta > 0 and currentPage > 0: #scroll up
#         selectChampionPageDict[currentPage].place_forget() 
#         currentPage -= 1
#         selectChampionPageDict[currentPage].place(relx = 210/1280, rely = 0.175)
...
# def pageCount(): #counts the number of pages needed
#     if championCount % 56 != 0:
#         return ((championCount // 56) + 1)
#     return championCount // 56
...
# currentPage = 0 #0 based indexing
# selectChampionPageDict = {}
# selectChampionLabelsDict = {}
# selectChampionTextDict = {}

# for pageCountIndex in range(pageCount()): #main code to show the champions in the pages as well as the pages
#     selectChampionPageDict[pageCountIndex] = Frame(selectChampionFrame, bg = colour87Black, width = 860, height = 600) #each element is 100x80 with 1st and last with indent from all dir. 40px for rows, 20px for col. Grid is 8 col, 7 rows
#     #selectChampionPageDict[currentPage].place(relx = 210/1280, rely = 0.175)
#     for selectChampionIndex in range(56):
#         if selectChampionIndex + 56 * (pageCountIndex) < championCount + 1:
#             #images
#             selectChampionImageDict[selectChampionIndex + 56 * (pageCountIndex)] = PhotoImage(file = champImagePath + j[selectChampionIndex + 56 * (pageCountIndex)] + ".png")
#             #labels
#             selectChampionLabelsDict[selectChampionIndex + 56 * (pageCountIndex)] = Label(selectChampionPageDict[pageCountIndex], bg = colour87Black, image = selectChampionImageDict[selectChampionIndex + 56 * (pageCountIndex)], cursor = "hand2")
#             selectChampionTextDict[selectChampionIndex + 56 * (pageCountIndex)] = Label(selectChampionPageDict[pageCountIndex], width = 10, height = 1, bg = colour87Black, text = j[selectChampionIndex + 56 * (pageCountIndex)], font = ("BeaufortforLOL-Bold", 9), anchor = "center", fg = championMasteryColourDecider(selectChampionIndex + 56 * (pageCountIndex), 1), cursor = "hand2")
#             #placings
#             selectChampionLabelsDict[selectChampionIndex + 56 * (pageCountIndex)].place(x = ((selectChampionIndex) % 8) * 100 + 60, y = ((selectChampionIndex) // 8) * 80 + 20)
#             selectChampionTextDict[selectChampionIndex + 56 * (pageCountIndex)].place(x = ((selectChampionIndex) % 8) * 100 + 45, y = ((selectChampionIndex) // 8) * 80 + 63)
#             #binds
#             selectChampionLabelsDict[selectChampionIndex + 56 * (pageCountIndex)].bind("<ButtonRelease-1>", displaySelectedChampion)
#             selectChampionTextDict[selectChampionIndex + 56 * (pageCountIndex)].bind("<ButtonRelease-1>", displaySelectedChampion)
#             selectChampionLabelsDict[selectChampionIndex + 56 * (pageCountIndex)].bind("<MouseWheel>", changePage)
#             selectChampionTextDict[selectChampionIndex + 56 * (pageCountIndex)].bind("<MouseWheel>", changePage)
#     selectChampionPageDict[pageCountIndex].bind("<MouseWheel>", changePage)
...
#Widgets
selectChampionSearchBox = Entry(selectChampionFrame, bg = colour90Black, fg = colourLeagueText, bd = 0, font = ("BeaufortforLOL-Bold", 24), width = 25, insertbackground = colourActuallyWhite)
...
#placements
selectChampionSearchBox.place(relx = 410/1280, rely = 278/720)
...
def search(list1, userInput): #search algorithm
    ...
    for i in range(len(list1)):
        if list1[i][0:len(userInput)].upper() == userInput.upper():
            list2.append("".ljust(15) + list1[i])
        elif userInput.upper() in list1[i].upper():
...
def selectChampionClickShared(frontID, eWidget):
    ...
    if widgetID == "":
        widgetID = 1
    actualID = int(widgetID) - int(frontID)
...
def selectChampionSearchBoxPressedShared(key): #search champion dynamic search algorithm
    ...
    #print(selectChampionSearchBox.get() + key)
    ...
    if searchList != "":
        for i in range(len(searchList)):
            ...
            selectChampionChampionsLabelDict[i] = Label(selectChampionDropdownFrameInner, bg = colour90Black, width = 36, height = 2, text = selectChampionChampionsDict[i], font = ("BeaufortforLOL-Bold", 16), fg = colourLeagueText, anchor = "w", cursor = "hand2")
    else:
        for i in range(len(j)):
            selectChampionChampionsDict[i] = j[i]
            selectChampionChampionsLabelDict[i] = Label(selectChampionDropdownFrameInner, bg = colour90Black, width = 36, height = 2, text = "".ljust(15) + selectChampionChampionsDict[i], font = ("BeaufortforLOL-Bold", 16), fg = colourLeagueText, anchor = "w", cursor = "hand2")
...
selectChampionFrame.bind("<ButtonRelease-1>", lambda e: selectChampionSearchBox.config(insertontime = 0))
